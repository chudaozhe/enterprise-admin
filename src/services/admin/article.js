...
export async function gets(category_id, keyword, page, max){
	const res = await http.get('/admin/'+getUserId()+'/category/'+category_id+'/article', {keyword:keyword, page:page, max:max});
...
export async function gets2(category_ids, keyword, page, max){
	const res = await http.get('/admin/'+getUserId()+'/article', {category_ids:category_ids.join(','), keyword:keyword, page:page, max:max});
	if(undefined!==res.data){
		return res.data.data;
	}else return res;
}
...
export async function add(data){
	...
	const res = await http.post('/admin/'+getUserId()+'/category/'+category_id+'/article', data);
	if(undefined!==res.data){
		return res.data;
	}else return res;
}
...
export async function edit(id, data){
	let category_id=data['category_id']??0;
	delete data.category_id;
	const res = await http.put('/admin/'+getUserId()+'/category/'+category_id+'/article/'+id, data);
	if(undefined!==res.data){
		return res.data;
	}else return res;
}

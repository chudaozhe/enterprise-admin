...
export async function gets(cid, keyword, page, max){
	const res = await http.get('/admin/'+getUserId()+'/filegroup/0/file', {'cid':cid, 'keyword':keyword, page:page, 'max':max});
...
export async function edit(id, data){
	const res = await http.put('/admin/'+getUserId()+'/filegroup/'+getUserId()+'/file/'+id, data);
	if(undefined!==res.data){
		return res.data;
	}else return res;
}

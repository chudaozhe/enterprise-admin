    ...
    <el-card>
      <template #header>
        <div class="card-header2">
          <div>
            <el-input size="small" v-model="keyword" placeholder="关键词" style="width:200px;"></el-input>
...
<script setup>
  ...
  const handleReset = async (id) => {
    ...
    if(result.err===0){
      instance.appContext.config.globalProperties.$message({type:'success', message:'已重置，新密码已发送到对应E-mail', duration:1000});
      document.querySelector("#app").click();
    }else{
      instance.appContext.config.globalProperties.$message({type:'error', message:result.msg, duration:1000});
    }
  ...
  const handleDisable = async (id) => {
    ...
    if(result.err===0){
      instance.appContext.config.globalProperties.$message({type:'success', message:'已禁用', duration:1000});
      //重新请求数据
  ...
  const handleEnable = async (id) => {
    ...
    if(result.err===0){
      instance.appContext.config.globalProperties.$message({type:'success', message:'已启用', duration:1000});
      //重新请求数据
      await handleList(keyword.value, 1, max.value);
      document.querySelector("#app").click();
    }else{
